...
1. Select the **Manage** hub, **Linked services**, then select **+ New**.
    ...
    ![The new linked service option is highlighted.](media/new-linked-service1.png "New linked service")
...
4. Select **OK** in the `Turn on data flow debug` dialog.
    ...
    ![The OK button is highlighted.](media/data-flow-debug-dialog1.png "Turn on data flow debug")
...
9. Enter each setting as displayed in the table below, and then select **OK**.
    ...
    ![The form is completed as described in the table below.](media/flightdelay-new-adls-properties1.png "Set properties")
...
13. For **Output stream name**, enter **FlightDelaysWithAirportCodes**, and select **+ New** next to **Source dataset**.
    ...
    ![The output stream name and new button are highlighted.](media/flightdelay-withairportcodes-source1.png "Source")
...
2. Under "Filter on", select **Open expression builder**.

    ![The enter filter box is highlighted.](media/flightdelay-filter-enter-filter1.png "Filter settings")
...
6. Under the **Connection** tab of the dataset properties, enter **aiaddw** SQL pool name for the **DBName** value. Select **Refresh** next to the table list, then select **dbo.DelaySummary** from the list.
   ...
   **IMPORTANT**: If the connection fails or if you are unable to get the table in the dropdown-menu.
   ...
   1. Navigate to the synapse workspace **synapselabinfraXXXXXX** select **Integration Settings** under Security tab.
   2. Check the **Allow pipelines (running as workspace's system assigned identity) to access SQL pools** option and click on **Save**. Then perform the above step again.
      ...
      ![managed-identity.](media/synapse-allow-pipelines1.png "managed identity")
...
4. Select the **Settings** tab, select **key_adls** from the **Staging linked service** list, then enter **datalake** in the container field and **polybase** in the folder field of the **Staging storage folder** setting.

    ![The settings form is configured as described.](media/flightdelay-pipeline-data-flow-settings1.png "Data flow settings")

5. Select **Publish all**, then select **Publish** in the dialog.

    ![Publish all is highlighted.](media/publish-all.png "Publish all")

6. Select **Add trigger**, then select **Trigger now**.

    ![The trigger now option is highlighted.](media/flightdelay-pipeline-trigger-now1.png "Trigger now")

7. Select **OK** to run the trigger.

    ![The OK button is highlighted.](media/import-data-trigger-pipeline-run.png "Pipeline run")

8. Select the **Monitor** hub, then **Pipeline runs**. If the pipeline run status is `Succeeded` after a few minutes, then the pipeline run successfully completed.
